...
**Comming Soon:**
<p align="left">
  <img src="docs/assets/peeky-pro.png" alt="Pro" width="300" />
</p>
...
- **Text mode.** Type a question without speaking. Press Enter to send. Useful in quiet rooms.
- **Document mode.** Attach a .txt, .md, or .pdf file from the right-click menu. Peeky splits it into chunks and answers questions about it. Everything stays in RAM - nothing is written to disk.
...
- **Memory timeline.** Every interaction is timestamped and stored locally. Browse the full history through the right-click menu.
...
| Document RAG   | Ollama embeddings    | Ollama embeddings       |
...
`install.bat` pulls the Python dependencies, the `gemma4:e4b` vision model (about 3 GB), and the `nomic-embed-text` embedding model (about 270 MB) used for document search. The first launch also downloads the faster-whisper base model (about 140 MB) for offline speech recognition. After that, everything is cached locally.
...
| Click Type            | Open a small dialog to type a question. Enter sends it.    |
| Click Video Coach     | Capture a baseline, describe the task, follow the loop.    |
| Right-click the agent | Copy reply, history, attach document, settings, quit.      |
| Drag the agent        | Move it anywhere on screen.                                |
...
Click the Screen button. The screen dims and a crosshair appears. Drag a rectangle around what you want to ask about. After releasing, the agent flips to recording mode. Speak your question and click to stop. The captured region and your question are sent to the local model together.

### Document mode

Right-click the widget and choose Attach document. Select any .txt, .md, or .pdf file. Peeky reads it, splits it into overlapping chunks, and embeds each chunk using the local `nomic-embed-text` model. When you ask a question, the most relevant chunks are retrieved and added to the prompt automatically.

The document lives only in memory. It is never written to disk. You can remove it at any time with Clear attachment from the right-click menu, or simply close the app.

If the `nomic-embed-text` model is not installed, Peeky falls back to keyword matching. The feature still works, just less precisely.

### Video Coach

Click Video Coach to start a coaching session. Peeky takes a baseline photo, then asks you to describe what you want to do. After you describe the task, Peeky:
...
Settings that can be changed from the right-click menu (saved to `settings.json`):

| Setting       | Default | Purpose                                           |
|---------------|---------|---------------------------------------------------|
| rag_topk      | 4       | Number of document chunks retrieved per question  |

## File layout

```
peeky.py             main application
peeky_settings.py    settings persistence and dialog
peeky_rag.py         ephemeral document RAG
...
settings.json        user preferences (created on first settings save)
...
- Online services are only used when you have an internet connection.
...
- Attached documents are kept only in RAM during the session. They are never written to disk.
...
**No microphone detected.** Open Windows Settings - Privacy and security - Microphone, and allow desktop apps to use it.
...
**Document RAG gives poor results.** Pull the embedding model: `ollama pull nomic-embed-text`. Without it, Peeky falls back to keyword matching which is less accurate.
